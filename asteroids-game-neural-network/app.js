...
const OUTPUT_LEFT = 0; // expected neural network output for turning left
const OUTPUT_RIGHT = 1; // expected neural network output for turning right
...
if (AUTOMATION_ON) {
    ...
    for (let i = 0; i < NUM_SAMPLES; i++) {
        ...
        // train the network
        nn.train(normalizeInput(asteroidx, asteroidy, angle, shipAngle), [direction]);
    }
}
...
function normalizeInput(roidx, roidy, roidAngle, shipAngle) {
...
function update() {
    ...
    if (AUTOMATION_ON) {
        ...
        let distance0 = distBetweenPoints(ship.x, ship.y, roids[0].x, roids[0].y);
        for (let i = 1; i < roids.length; i++) {
            let distance1 = distBetweenPoints(ship.x, ship.y, roids[i].x, roids[i].y);
        ...
        let asteroidx = roids[c].x;
        let asteroidy = roids[c].y;
        let shipAngle = ship.a;
        let shipx = ship.x;
        let shipy = ship.y;
        let angle = angleToPoint(shipx, shipy, shipAngle, asteroidx, asteroidy);
        let predict = nn.feedForward(normalizeInput(asteroidx, asteroidy, angle, shipAngle)).data[0][0];

        // make a turn
        let deltaLeft = Math.abs(predict - OUTPUT_LEFT);
        let deltaRight = Math.abs(predict - OUTPUT_RIGHT);
